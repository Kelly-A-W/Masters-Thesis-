...
combo_cd4 <- function(ws, pid, days, comb=comb_final){
  ...
  for(i in 1:length(days)){
    gates <- flowjo_to_gatingset(ws, name = (i+1))
    day   <- days[i]
    ...
    for(j in 1:length(stim)){
      ...
      if("try-error" %in% class(try(gates[[j]], silent = T))){
        ...
        G      <- gh_pop_get_indices(gh, "CD4+/IFN+")[cd4_in]
        L2     <- gh_pop_get_indices(gh, "CD4+/IL2+")[cd4_in]
  ...
  for(i in 1:length(days)){
    ...
    for(j in 1:length(stim)){
      ...
      if("try-error" %in% class(try(gates[[j]], silent = T))){
        ...
        G      <- gh_pop_get_indices(gh, "CD8+/IFN+")[cd8_in]
        L2     <- gh_pop_get_indices(gh, "CD8+/IL2+")[cd8_in]
...
setwd("/Volumes/Elements/WBA Workspace & Fcs Final/H56-032/G3 QFT pos")

my_days <- c(0,126,210)
# UNS|Ag85B|ESAT6|PHA
my_pid  <- "10309"                               ####### CHANGE #######
my_ws   <- open_flowjo_xml(paste0(my_pid, ".wsp")) 
#my_ws   <- open_flowjo_xml("1118_AG.wsp")
my_ws 
...
setwd("/Users/kellywilliams/Documents/Masters Thesis/FlowJo Booleans Dataset 1/H56-032/Data")
