...
combo_cd4 <- function(ws, pids, days, comb=comb_final){
  out <- data.frame()
  ...
  for(i in 1:length(days)){
    ...
    for(j in 1:length(stim)){
      ...
      if("try-error" %in% class(try(gates[[j]], silent = T))){
        ...
        L17    <- gh_pop_get_indices(gh, "CD4+/IL-17+")[cd4_in]
...
combo_cd8 <- function(ws, pids, days, comb=comb_final){
  out <- data.frame()
  ...
  for(i in 1:length(days)){
    ...
    for(j in 1:length(stim)){
      ...
      if("try-error" %in% class(try(gates[[j]], silent = T))){
        ...
        L17    <- gh_pop_get_indices(gh, "CD8+/IL-17+")[cd8_in]
...
# UNS|Ag85B|ESAT6|PHA
                         ####### CHANGE #######
date <- 22122015
#my_ws   <- open_flowjo_xml(paste0(my_pid, "_AG_Final.wsp")) 
my_ws   <- open_flowjo_xml("035-456 22122015_AG_Final.wsp")
my_ws 
my_days <- c(292, 292, 292, 292, 292, 292, 292, 292, 292, 292)
my_pid  <- c(1021, 1022, 1024, 1025, 1026, 1027, 1028, 1029, 1030, 1031)   


counts_cd4 <- combo_cd4(ws=my_ws, pids=my_pid, days=my_days)
freq_cd4   <- counts_cd4
freq_cd4   <- freq_cd4 %>%
...
counts_cd8 <- combo_cd8(ws=my_ws, pids=my_pid, days=my_days)
freq_cd8   <- counts_cd8
freq_cd8   <- freq_cd8 %>%
...
setwd("/Users/kellywilliams/Documents/Masters Thesis/FlowJo Gating/H56-035/Data")

write.csv(counts_cd4, paste0("cd4_counts_date_",date, ".csv"), row.names = FALSE) 
write.csv(freq_cd4, paste0("cd4_frequency_date_",date, ".csv"), row.names = FALSE) 

write.csv(counts_cd8, paste0("cd8_counts_date_",date, ".csv"), row.names = FALSE)  
write.csv(freq_cd8, paste0("cd8_frequency_date_",date, ".csv"), row.names = FALSE) 
