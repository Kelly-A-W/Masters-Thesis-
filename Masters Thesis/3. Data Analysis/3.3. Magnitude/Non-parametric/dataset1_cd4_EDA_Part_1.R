...
# Calculate TVR
#---------------------
# We want the total vaccine response
# So some TRFs of Ag85B and ESAT6
TVR <- cd4_bstrf$Total.Ag85B + cd4_bstrf$Total.ESAT6
...
stat.test <- stat.test %>%
  add_xy_position(x = "Dose", dodge = 0.8)
stat.test$y.position[-1] <- c(3.9, 4.2, 4.5, 3.2, 3.5, 3.8)
# Calculate adjusted p-values:
(stat.test$p.adj <-round(p.adjust(stat.test$p, method = "BH"),3))
# Create Plot:
ggplot(dftest, aes(y=AUC, x=Dose)) +
