...
ind1 <- find_cell(tab, 2, 3, "core-fg")
ind2 <- find_cell(tab, 2, 2, "core-fg")
...
# Adjusted P-values:
(padj      <- round(p.adjust(stat.test$p, method = "BH"),3))
padj[4]    <- round(p.adjust(stat.test$p, method = "BH"),4)[4]   # don't want 0
padj[7]    <- round(p.adjust(stat.test$p, method = "BH"),4)[7]   # don't want 0
# Create Plot: 
...
ind3 <- find_cell(tab, 3, 3, "core-fg")
tab$grobs[ind1][[1]][["gp"]] <- gpar(fontsize=25, fontface="bold")
tab$grobs[ind2][[1]][["gp"]] <- gpar(fontsize=25, fontface="bold")
...
#stat.test$y.position <- c(2.7, 2.7, 2.9, 3.1, 2.7, 2.9, 3.1)
# Calculate adjusted p-values:
(stat.test$p.adj <-round(p.adjust(stat.test$p, method = "BH"),3))
stat.test$p.adj[3]    <- round(p.adjust(stat.test$p, method = "BH"),4)[3]   # don't want 0
# Create Plot:
...
stat.test$y.position <- c(1.75, 1.9, 2.05, 1.8, 1.95, 2.1)
# Calculate adjusted p-values:
(stat.test$p.adj <-round(p.adjust(stat.test$p, method = "BH"),3))
# Create Plot:
