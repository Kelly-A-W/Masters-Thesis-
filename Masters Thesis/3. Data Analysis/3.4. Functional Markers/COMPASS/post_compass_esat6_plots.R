...
tab         <- tableGrob(df1[,-1], rows=NULL, theme = ttheme_default(base_size = 25))
# Set significant p-values to bold:
ind1 <- find_cell(tab, 3, 1, "core-fg")
tab$grobs[ind1][[1]][["gp"]] <- gpar(fontsize=25, fontface="bold")
ind1 <- find_cell(tab, 4, 1, "core-fg")
tab$grobs[ind1][[1]][["gp"]] <- gpar(fontsize=25, fontface="bold")
# Set widths/heights to 'fill whatever space I have':
tab$widths  <- unit(rep(1, ncol(tab)), "null")
tab$heights <- unit(rep(1, nrow(tab)), "null")
...
(stat.test$p.adj <-p.adjust(stat.test$p, method = "BH"))
stat.test$p.adj = c("0.0001", "<0.00001", "0.0001")
# Create Plot:
...
ggplot(dftest, aes(y=FS, x=Dose)) +
  ...
  stat_pvalue_manual(stat.test, 
                     ...
                     size = 10, 
                     bracket.size = 0.8,
                     fontface=c("bold", "plain", "plain", "bold", "plain", "plain",  "plain", "plain", "plain"))
...
# Calculate adjusted p-values:
(stat.test$p.adj <-p.adjust(stat.test$p, method = "BH"))
stat.test$p.adj = c("0.0001", "<0.00001", "0.001")
# Create Plot:
...
# Calculate adjusted p-values:
(stat.test$p.adj <-round(p.adjust(stat.test$p, method = "BH"),3))
stat.test$y.position = c(0.63, 0.7, 0.65, 0.66, 0.73, 0.68)
# Create Plot:
...
# Calculate adjusted p-values:
(stat.test$p.adj <-round(p.adjust(stat.test$p, method = "BH"),3))
stat.test$y.position = c(0.54, 0.64, 0.58, 0.54, 0.64, 0.58)
# Create Plot:
ggplot(dftest, aes(y=FS, x=QFT)) +
  geom_boxplot(alpha=1, size =1, aes(fill = Dose)) +
  theme_minimal()+
...
# Calculate adjusted p-values:
(stat.test$p.adj <-round(p.adjust(stat.test$p, method = "BH"),3))
stat.test$y.position = c(0.66, 0.75, 0.7, 0.6, 0.69, 0.64)
# Create Plot:
...
# Calculate adjusted p-values:
(stat.test$p.adj <-round(p.adjust(stat.test$p, method = "BH"),3))
stat.test$y.position = c(0.62, 0.69, 0.64, 0.53, 0.68, 0.63)
# Create Plot:
ggplot(dftest, aes(y=PFS, x=QFT)) +
  geom_boxplot(alpha=1, size =1, aes(fill = Dose)) +
  theme_minimal()+
