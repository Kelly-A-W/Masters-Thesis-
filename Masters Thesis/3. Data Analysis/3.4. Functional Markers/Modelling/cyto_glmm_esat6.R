...
esat6_df <- read.csv("Esat6_compass_filtered_data_for_mods.csv", check.names = F)
str(esat6_df)

#---------------------
# Set Factors
#---------------------
esat6_df$ethnicity = as.factor(esat6_df$ethnicity)
esat6_df$sex = as.factor(esat6_df$sex)
esat6_df$timepnt = as.factor(esat6_df$timepnt)
esat6_df$QFT = as.factor(esat6_df$QFT)
esat6_df$PID = as.factor(esat6_df$PID)
esat6_df$Marker = as.factor(esat6_df$Marker)

esat6_df$timepnt <- factor(esat6_df$timepnt, levels=c("baseline", "peak", "memory"))
levels(esat6_df$timepnt)
...
mod4 <- stepGAIC(mod4.0, 
                 scope = list(lower= ~ 1,
                              upper = ~ QFT + Marker + Schedule + Dose+timepnt), # For some reason timepnt results in algorithm not converging, so i'm just going to try other variables
                 k=round(log(length(df1$value)), 2), 
                 what = "sigma")
...
p=ggplot(df2, aes(y=residuals, x=Marker)) +
  ...
  scale_fill_manual(values = brewer.pal(n = 7, name = "BuGn"))
# Make grobtable:
