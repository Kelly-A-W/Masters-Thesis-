...
df <- esat6_mrf
df$R7..RAc.. <- log(df$R7..RAc.. + 0.001) # fitting a lognormal distr
...
base_model <-gamlss(R7..RAc.. ~ 
                    ...
                    data=df1, 
                    family=NO(mu.link=identity, sigma.link = identity)) # NB identity link because lmm
...
GAIC(mod3, k=round(log(length(df1$R7..RAc..)), 2)) # choose mod3
...
summary(mod3)
getSmo(mod3) 
GAIC(mod3, k=round(log(length(df1$R7..RAc..)), 2))
