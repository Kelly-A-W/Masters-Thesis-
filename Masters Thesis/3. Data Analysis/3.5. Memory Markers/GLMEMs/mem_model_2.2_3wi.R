...
# GAIC:
GAIC(int_model, k=round(log(length(df1$R7..RAc..)), 2))    # choose int_model
GAIC(mod_slo, k=round(log(length(df1$R7..RAc..)), 2)) 
...
mod1 <- stepGAIC(int_model, 
                 scope = list(lower= ~ QFT + Schedule + Dose + timepnt + Stim + re(random=~1|PID, method = "REML"),
                              upper = ~ QFT + Schedule + Dose + timepnt + Stim + age + ethnicity + sex + Study + Day + re(random=~1|PID, method = "REML")),
                 k=round(log(length(df1$R7..RAc..)), 2))
summary(mod1)
...
mod2 <- stepGAIC(mod1, 
                 scope = list(lower= ~ QFT + Schedule + Dose + timepnt + Stim + re(random=~1|PID, method = "REML"),
                              upper = ~ Schedule*Dose + timepnt*Schedule + QFT*Dose + QFT*Schedule + QFT*timepnt +
                                Dose*timepnt + Stim*QFT + Stim*Schedule + Stim*Dose + Stim*timepnt + re(random=~1|PID, method = "REML")),
                 k=round(log(length(df1$R7..RAc..)), 2))
...
GAIC(mod2, k=round(log(length(df1$R7..RAc..)), 2))  # choose mod2
GAIC(mod3.1, k=round(log(length(df1$R7..RAc..)), 2)) 
...
mod4 <- stepGAIC(mod4.0, 
                 scope = list(lower= ~ 1,
                              upper = ~ QFT + Stim + Schedule + Dose), # For some reason timepnt results in algorithm not converging, so i'm just going to try other variables
                 k=round(log(length(df1$R7..RAc..)), 2), 
                 what = "sigma")
summary(mod4) # nothing selected
...
GAIC(mod2, k=round(log(length(df1$R7..RAc..)), 2))
...
ggplot(df2, aes(y=residuals, x=Dose)) +
  ...
  scale_fill_manual(values = brewer.pal(n = 3, name = "BuGn"))
# Residuals vs Stim:
df2 <- data.frame(residuals = resid(mod2),  Stim = df1$Stim)
...
# QFT*Stim:
df2 <- data.frame(residuals = resid(mod2),  Stim = df1$Stim, QFT=df1$QFT)
ggplot(df2, aes(y=residuals, x=Stim)) +
  geom_boxplot(alpha=1, size =1, aes(fill = QFT), color ="black") +
  theme_minimal()+
  theme(text = element_text(size = 30)) +
  ...
  scale_fill_manual(values=c("#FF0000", "blue"), labels = c("QFT-", "QFT+"), name="")
# Stim*timepnt:
df2 <- data.frame(residuals = resid(mod2),  time = df1$timepnt, Stim=df1$Stim)
ggplot(df2, aes(y=residuals, x=time)) +
  geom_boxplot(alpha=1, size =1, aes(fill = Stim), color ="black") +
  theme_minimal()+
  theme(text = element_text(size = 30)) +
