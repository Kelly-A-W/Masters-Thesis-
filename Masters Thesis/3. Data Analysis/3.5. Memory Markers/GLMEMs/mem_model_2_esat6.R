...
esat6$Marker = as.factor(esat6$Marker)
...
# Marker*timepnt:
df2 <- data.frame(residuals = resid(mod3),  time = df1$timepnt, Marker=df1$Marker)
ggplot(df2, aes(y=residuals, x=time)) +
  geom_boxplot(alpha=1, size =1, aes(fill = Marker), color ="black") +
  theme_minimal()+
  theme(text = element_text(size = 30)) +
  ylab("Normalized quantile residuals") +
  xlab(NULL)+
  scale_fill_manual(values = brewer.pal(n = 3, name = "BuGn"), labels = c("CCR7+CD45RA-", "CCR7-CD45RA-"), name="")
