...
# only want the 2 marker combos:
cd4_mrf_2 <- cd4_mrf[,-c(13,15)]



# Three-Level Grouping:
# fit only one model
long <- melt(setDT(cd4_mrf_2), id.vars = c("age", "ethnicity", "sex", "Schedule","Dose", 
                                           "timepnt", "Study", "QFT", "Group", "PID", "Day", 
                                           "Stim"),
...
mod_slo1 <-gamlss(value ~ 
                    ...
                    Stim +
                    Marker +
                    re(random=~Stim|PID, method = "REML", control = nlmeControl(opt = "nlm")),  
                  data=df1, 
                  family=WEI(mu.link=log, sigma.link = identity)) # NB identity link because lmm

mod_slo2 <-gamlss(value ~ 
                    QFT + 
                    Schedule + 
                    Dose +
                    timepnt +
                    Stim +
                    Marker +
                    re(random=~Marker|PID, method = "REML", control = nlmeControl(opt = "nlm")),  
                  data=df1, 
                  family=WEI(mu.link=log, sigma.link = identity)) # NB identity link because lmm

mod_slo3 <-gamlss(value ~ 
                    QFT + 
                    Schedule + 
                    Dose +
                    timepnt +
                    Stim +
                    Marker +
                    re(random=~Stim+Marker|PID, method = "REML", control = nlmeControl(opt = "nlm")),  
                  data=df1, 
                  family=WEI(mu.link=log, sigma.link = identity)) # NB identity link because lmm
...
GAIC(mod_slo1, k=round(log(length(df1$value)), 2)) 
GAIC(mod_slo2, k=round(log(length(df1$value)), 2)) 
GAIC(mod_slo3, k=round(log(length(df1$value)), 2)) 
...
mod1 <- stepGAIC(int_model, 
                 scope = list(lower= ~ QFT + Schedule + Dose + timepnt + Stim + Marker + re(random=~1|PID, method = "REML"),
                              upper = ~ QFT + Schedule + Dose + timepnt + Stim + age + ethnicity + sex + Study + Day + Marker + re(random=~1|PID, method = "REML")),
...
mod2 <- stepGAIC(mod1, 
                 scope = list(lower= ~ QFT + Schedule + Dose + timepnt + Stim + Marker +re(random=~1|PID, method = "REML"),
                              upper = ~ Schedule*Dose + timepnt*Schedule + QFT*Dose + QFT*Schedule + QFT*timepnt +
                                Dose*timepnt + Stim*QFT + Stim*Schedule + Stim*Dose + Stim*timepnt + 
                                Marker*QFT + Marker*Schedule + Marker*Dose + Marker*timepnt + Marker*Stim +re(random=~1|PID, method = "REML")),
                 k=round(log(length(df1$value)), 2))
summary(mod2)
...
mod3.2 <-update(mod2, ~.+QFT*Stim*timepnt)
mod3.3 <-update(mod2, ~.+QFT*Schedule*timepnt)
mod3.4 <-update(mod2, ~.+QFT*Stim*timepnt)
mod3.5 <-update(mod2, ~.+Dose*Stim*QFT)
mod3.6 <-update(mod2, ~.+Dose*Stim*Marker)
mod3.7 <-update(mod2, ~.+Dose*Stim*timepnt)
mod3.8 <-update(mod2, ~.+QFT*Marker*Stim)
mod3.9 <-update(mod2, ~.+timepnt*Marker*Stim)
mod3.10 <-update(mod2, ~.+Schedule*timepnt*Marker)
mod3.11 <-update(mod2, ~.+Schedule*timepnt*Stim)
...
GAIC(mod3.4, k=round(log(length(df1$value)), 2)) 
GAIC(mod3.5, k=round(log(length(df1$value)), 2)) 
GAIC(mod3.6, k=round(log(length(df1$value)), 2)) 
GAIC(mod3.7, k=round(log(length(df1$value)), 2)) 
GAIC(mod3.8, k=round(log(length(df1$value)), 2)) 
GAIC(mod3.9, k=round(log(length(df1$value)), 2)) 
GAIC(mod3.10, k=round(log(length(df1$value)), 2)) 
GAIC(mod3.11, k=round(log(length(df1$value)), 2)) 
...
mod3 <- stepGAIC(mod3.0, 
                 scope = list(lower= ~ 1,
                              upper = ~ QFT + Stim + Schedule + Dose + Marker), # For some reason timepnt results in algorithm not converging, so i'm just going to try other variables
                 k=round(log(length(df1$value)), 2), 
                 what = "sigma")
summary(mod3) # Marker and stim selected
...
ggplot(df2, aes(y=residuals, x=Dose)) +
  ...
  scale_fill_manual(values = brewer.pal(n = 3, name = "BuGn"))
# Residuals vs Stim:
df2 <- data.frame(residuals = resid(mod3),  Stim = df1$Stim)
df2$Stim <- as.factor(df2$Stim)
ggplot(df2, aes(y=residuals, x=Stim)) +
  geom_boxplot(alpha=1, size =1, aes(fill = Stim), color ="black") +
  theme_minimal()+
  theme(text = element_text(size = 30), legend.position = "none") +
  ylab("Normalized quantile residuals") +
  xlab(NULL)+
